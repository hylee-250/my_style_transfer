...
class GenerSpeech(FastSpeech2):
    ...
    def forward(self, txt_tokens, mel2ph=None, ref_mel2ph=None, ref_mel2word=None, spk_embed=None, emo_embed=None, ref_mels=None,
        ...
        ret = {}
        # self.encoder -> Fastspeech encoder
        ...
        decoder_inp = self.norm(decoder_inp)
        ...
        if skip_decoder:
            return ret
        mel_out = self.run_decoder(decoder_inp, tgt_nonpadding, ret, infer=infer, **kwargs)

        # postflow
        is_training = self.training
        x_mask = tgt_nonpadding
        ref_prosody = prosody_utter_mel + prosody_ph_mel + prosody_word_mel
        if not infer:
            if is_training:
                z_pf, ldj_pf, postflow = self.run_post_glow(ref_mels, infer, is_training, ret)
            
            return ref_mel2ph, ref_mel2word, decoder_inp, mel_out, x_mask, spk_embed, emo_embed, ref_prosody,z_pf, ldj_pf, postflow
        else:
            mel_out = self.run_post_glow(ref_mels, infer, is_training, ret)
            return ref_mel2ph, ref_mel2word, decoder_inp, mel_out, x_mask, spk_embed, emo_embed, ref_prosody 
    ...
    def run_post_glow(self, tgt_mels, infer, is_training, mel_out, decoder_inp,spk_embed,emo_embed,ref_prosody,x_mask, ret):
        x_recon = mel_out.transpose(1, 2)
        g = x_recon
        B, _, T = g.shape
        if hparams.get('use_txt_cond', True):
            g = torch.cat([g, decoder_inp.transpose(1, 2)], 1)
        g_spk_embed = spk_embed.repeat(1, T, 1).transpose(1, 2)
        g_emo_embed = emo_embed.repeat(1, T, 1).transpose(1, 2)
        l_ref_prosody = ref_prosody.transpose(1, 2)
